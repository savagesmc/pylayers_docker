...
if [ $(id -u) == 0 ] ; then
    ...
    usermod -d /home/$NB_USER -l $NB_USER pylayers
    ...
    if [[ "$NB_USER" != "pylayers" ]]; then
        # changing username, make sure homedir exists
        # (it could be mounted, and we shouldn't create it if it already exists)
        if [[ ! -e "/home/$NB_USER" ]]; then
            echo "Relocating home dir to /home/$NB_USER"
            mv /home/pylayers "/home/$NB_USER"
        fi
        # if workdir is in /home/pylayers, cd to /home/$NB_USER
        if [[ "$PWD/" == "/home/pylayers/"* ]]; then
    ...
    if [[ "$NB_UID" == "$(id -u pylayers)" && "$NB_GID" == "$(id -g pylayers)" ]]; then
	...
	if [[ "$STATUS" != "0" ]]; then
            if [[ -w /etc/passwd ]]; then
                ...
                cat /etc/passwd | sed -e "s/^pylayers:/nayvoj:/" > /tmp/passwd
                echo "pylayers:x:$(id -u):$(id -g):,,,:/home/pylayers:/bin/bash" >> /tmp/passwd
                cat /tmp/passwd > /etc/passwd
                rm /tmp/passwd
                id -G -n 2>/dev/null | grep -q -w $(id -u) || STATUS=$? && true
                if [[ "$STATUS" != "0" && "$(id -g)" == "0" ]]; then
                    echo "Adding group file entry for $(id -u)"
                    echo "pylayers:x:$(id -u):" >> /etc/group
                fi
            else
